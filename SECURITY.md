...
Important security properties include fail-closed escalation, context binding, finite cache expiration, private local state modes, child-path symlink rejection, receipt-chain integrity, and avoiding unintended model or network calls. Corrupt or schema-incompatible cache state cannot produce cache hits; cache-dependent routes escalate as `cache_unavailable`. Transient SQLite contention does not permanently disable later cache access. Evidence digests are caller attestations and do not prove semantic truth. Keyword screening is defense in depth; callers must declare side effects explicitly.
